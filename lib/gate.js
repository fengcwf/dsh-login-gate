...
export function createGateServer(o) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (path === '/__gate/login') {
      if (req.method === 'POST') {
        ...
        if (!ok) {
          ...
          log?.(`[login-gate] 登录失败：user=${user || '(空)'} ip=${ip}`)
        ...
        log?.(`[login-gate] 登录成功：user=${user} ip=${ip}`)
    ...
    if (path === '/__gate/logout-all' && req.method === 'POST') {
      ...
      onLogoutAll()
      log?.(`[login-gate] user=${session.u} 执行 logout-all：全部会话已作废`)
      return redirect(res, '/__gate/login', clearCookie())
    }
  ...
  server.on('upgrade', async (req, socket) => {
    ...
    try {
      await forwarder.forwardUpgrade(req, socket, Buffer.alloc(0))
    } catch {
      socket.destroy()
    }
  })
